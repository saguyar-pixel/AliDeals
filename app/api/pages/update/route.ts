...
export async function POST(req: NextRequest) {
  try {
    ...
    const updatedPage = {
      ...
      archetype: data.archetype || current?.archetype || undefined,
      pros: Array.isArray(data.pros) ? data.pros : current?.pros || [],
      cons: Array.isArray(data.cons) ? data.cons : current?.cons || [],
      isEuPlug: data.isEuPlug !== undefined ? data.isEuPlug : current?.isEuPlug ?? null,
      voltage220vCompatible: data.voltage220vCompatible !== undefined ? data.voltage220vCompatible : current?.voltage220vCompatible ?? null,
      sizeWarning: data.sizeWarning !== undefined ? data.sizeWarning : current?.sizeWarning ?? null,
      fabricComposition: data.fabricComposition !== undefined ? data.fabricComposition : current?.fabricComposition ?? null,
