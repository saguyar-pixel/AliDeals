...
export default async function DealPage({ params }: DealPageProps) {
  ...
  const faqs = safeParse<Array<{ question: string; answer: string }>>(
    ...
    [
      ...
      {
        question: prod?.archetype === "ELECTRONICS"
          ? "איך מבטיחים קבלת שקע שמתאים לישראל?"
          : prod?.archetype === "FASHION"
          ? "איך המידות במוצר זה ביחס למידות בישראל?"
          : "האם המוצר מגיע עם הגנת קונה ואחריות משלוח?",
        answer: prod?.archetype === "ELECTRONICS"
          ? "בעת ביצוע ההזמנה בעלי אקספרס יש לבחור באפשרות תקע EU Plug (שקע אירופאי), המתאים בדיוק לשקעים בישראל ללא מתאם."
          : prod?.archetype === "FASHION"
          ? "המידות הן מידות אסייתיות, מומלץ לבדוק את טבלת המידות בס\"מ ולהזמין לרוב מידה אחת מעל המידה הרגילה בישראל."
          : "כן, הרכישה כוללת הגנת קונה מלאה של AliExpress המבטיחה החזר כספי במקרה של אובדן או אי-התאמה למפרט.",
      },
      {
