...
export default function AgentTeamPage() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("alideals_agent_messages");
      if (saved) {
  ...
  useEffect(() => {
    if (messages.length > 1) {
      try {
        localStorage.setItem("alideals_agent_messages", JSON.stringify(messages));
      } catch {}
    }
  }, [messages]);
  ...
  const fetchStatus = async () => {
    try {
      ...
      if (data.messages && Array.isArray(data.messages)) {
        setMessages((prev) => {
          // If serverless container returns only default welcome message (length <= 1)
          // but client already has active user conversation, do NOT overwrite and lose conversation!
          if (prev.length > 1 && data.messages.length <= 1) {
            return prev;
          }
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    // Optimistic UI update: Immediately append user message to chat
    ...
    try {
      ...
      if (data.messages && Array.isArray(data.messages) && data.messages.length > 0) {
        ...
        try {
          localStorage.setItem("alideals_agent_messages", JSON.stringify(data.messages));
        } catch {}
      } else if (data.reply) {
        const alonMsg: OrchestratorMessage = {
          id: `temp_alon_${Date.now()}`,
          sender: "orchestrator",
          text: data.reply,
          timestamp: new Date().toLocaleTimeString("he-IL", { hour12: false }),
        };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 flex flex-col rounded-3xl bg-white border border-slate-200 shadow-sm overflow-hidden h-[620px]">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {geminiConfigured ? (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
                        ...
                        <span>Gemini 2.5 Flash מחובר</span>
                      </span>
                    ) : (
                      <span
                        className="inline-flex items-center gap-1 text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200 cursor-pointer hover:bg-indigo-50 hover:text-indigo-700 transition-colors"
                        title="הדבק מפתח Gemini (AIzaSy...) בצ'אט כדי להפעיל את מוח ה-AI הגנרטיבי המלא"
                        onClick={() => setInputText("מי אתה ומה היכולות שלך?")}
                      >
                        <Bot className="w-3 h-3 text-slate-400" />
                        <span>מוח בסיסי (הדבק מפתח ל-AI מלא)</span>
                      </span>
                    )}
                  </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 text-xs sm:text-sm">
            ...
            {isSending && (
              <div className="flex items-start gap-2.5 flex-row animate-in fade-in duration-200">
                <div className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs shrink-0 bg-indigo-600 text-white animate-pulse shadow-xs">
                  אלון
                </div>
                <div className="bg-slate-100 text-slate-700 rounded-2xl rounded-tl-none p-3 border border-slate-200 flex items-center gap-2">
                  <RefreshCw className="w-3.5 h-3.5 animate-spin text-indigo-600" />
                  <span className="text-xs font-medium">אלון חושב ומפעיל את צוות הסוכנים...</span>
                </div>
              </div>
            )}
          ...
          <div className="px-3 py-2 bg-slate-50 border-t border-slate-200 flex items-center gap-1.5 overflow-x-auto text-[11px]">
            ...
            <button
              type="button"
              onClick={() => setInputText("תפעיל לופ אוטונומי")}
              className="px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-indigo-700 hover:bg-indigo-50 shrink-0 font-bold"
            >
              🚀 הפעל לופ אוטונומי
            </button>
            <button
