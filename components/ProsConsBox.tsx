...
export default function ProsConsBox({ pros = [], cons = [] }: ProsConsBoxProps) {
  const validPros = (pros || []).filter((p) => Boolean(p && typeof p === "string" && p.trim()));
  const validCons = (cons || []).filter((c) => Boolean(c && typeof c === "string" && c.trim()));

  if (validPros.length === 0 && validCons.length === 0) {
    return null;
  }

  const isSingleColumn = validPros.length === 0 || validCons.length === 0;

  return (
    <div
      className={`my-8 grid grid-cols-1 ${
        isSingleColumn ? "max-w-2xl mx-auto" : "md:grid-cols-2"
      } gap-6`}
      dir="rtl"
    >
      {/* Pros Card */}
