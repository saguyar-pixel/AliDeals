      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-200 bg-white/95 backdrop-blur shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            {/* Desktop Navigation — rendered server-side, zero hydration cost */}
            <nav className="hidden md:flex items-center gap-1 font-medium text-sm text-slate-700">
              <Link href="/" className="px-3.5 py-2 rounded-lg hover:bg-slate-100 hover:text-ali-600 transition-colors">
                ראשי
              </Link>

              {/* TOP 5 Dropdown — CSS hover only, no JS */}
              <div className="relative group">
                <Link
                  href="/#top5"
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg hover:bg-slate-100 hover:text-ali-600 transition-colors"
                ...
                <div className="absolute top-full right-0 mt-1 w-72 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 hidden group-hover:block z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                  {top5Items.length > 0 ? (
                    ...
                    <div className="p-3 text-xs text-slate-500 text-center">מדריכי TOP 5 מעודכנים יופיעו כאן</div>
                  )}
                  <div className="border-t border-slate-100 mt-1 pt-1">
                    <Link href="/#top5" className="block text-center py-2 text-xs font-semibold text-ali-600 hover:bg-ali-50 rounded-lg transition-colors">
                      לכל טבלאות ההשוואה באתר ←
                    </Link>
                  </div>
                </div>
              ...
              <Link href="/#reviews" className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg hover:bg-slate-100 hover:text-ali-600 transition-colors">
                <Star className="w-4 h-4 text-amber-500" />
                סקירות עומק
              </Link>

              <Link href="/#deals" className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg hover:bg-slate-100 hover:text-ali-600 transition-colors">
                <Flame className="w-4 h-4 text-ali-500" />
                דילים חמים
              </Link>

              {/* Dynamic CMS Header Links */}
              {customHeaderLinks.map((item) => {
                if (item.isDropdown && item.children && item.children.length > 0) {
                  return (
                    <div key={item.id} className="relative group">
                      <Link href={item.href || "#"} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg hover:bg-slate-100 hover:text-ali-600 transition-colors">
                        {item.icon && <span>{item.icon}</span>}
                        <span>{item.title}</span>
                        <ChevronDown className="w-3.5 h-3.5 text-slate-400 group-hover:rotate-180 transition-transform" />
                      </Link>
                      <div className="absolute top-full right-0 mt-1 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 hidden group-hover:block z-50">
                        {item.children.map((child) => (
                          <Link key={child.id} href={child.href} className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-slate-50 transition-colors text-slate-800">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {child.subtitle && <div className="text-[11px] text-slate-500 truncate">{child.subtitle}</div>}
                            </div>
                          </Link>
                        ))}
                      </div>
                ...
                return (
                  <Link key={item.id} href={item.href} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg hover:bg-slate-100 hover:text-ali-600 transition-colors">
                    {item.icon && <span>{item.icon}</span>}
                    <span>{item.title}</span>
                  </Link>
                );
            ...
            {/* Interactive parts: search, mobile menu, LiveSearchModal */}
            <HeaderInteractive top5Items={top5Items} customHeaderLinks={customHeaderLinks} />
          </div>
        </div>
      </header>
    </>
...
// ─── HeaderInteractive (Client Component) ────────────────────────────────────
// Handles: mobile menu, search modal, Ctrl+K hotkey, lazy LiveSearchModal.
// Nav data is passed as props from the server-rendered Header above.
// ─────────────────────────────────────────────────────────────────────────────
...
import dynamic from "next/dynamic";
import {
  ShieldCheck, Settings, Menu, X, ArrowLeft, Search, Star, Flame, Layers,
} from "lucide-react";
...
interface InteractiveNavItem {
  ...
  isActive: boolean;
  isDropdown?: boolean;
  children?: Array<{ id: string; title: string; href: string; icon?: string; subtitle?: string; order: number }>;
}

interface HeaderInteractiveProps {
  top5Items: InteractiveNavItem[];
  customHeaderLinks: InteractiveNavItem[];
}

export default function HeaderInteractive({ top5Items, customHeaderLinks }: HeaderInteractiveProps) {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [searchModalOpen, setSearchModalOpen] = useState(false);
  ...
  return (
    <>
      ...
      {/* Mobile Dropdown — fixed positioning to prevent CLS on the sticky header */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed top-16 inset-x-0 z-40 border-t border-slate-200 bg-white px-4 py-4 space-y-2 shadow-xl animate-in slide-in-from-top-2 duration-200" dir="rtl">
          <button
            onClick={() => { setMobileMenuOpen(false); setSearchModalOpen(true); }}
            className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-ali-50/80 border border-ali-200 text-ali-900 font-bold text-sm mb-2"
          >
            <span className="flex items-center gap-2"><Search className="w-4 h-4 text-ali-600" /><span>חיפוש חי באלי אקספרס...</span></span>
            <ArrowLeft className="w-4 h-4 text-ali-500" />
          </button>

          <Link href="/" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-3 rounded-xl hover:bg-slate-50 text-slate-900 font-semibold text-sm">
            <span>ראשי</span><ArrowLeft className="w-4 h-4 text-slate-400" />
          </Link>
          <Link href="/#reviews" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-3 rounded-xl hover:bg-slate-50 text-slate-900 font-semibold text-sm">
            <span className="flex items-center gap-2"><Star className="w-4 h-4 text-amber-500" /><span>סקירות עומק</span></span>
            <ArrowLeft className="w-4 h-4 text-slate-400" />
          </Link>
          <Link href="/#top5" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-3 rounded-xl hover:bg-slate-50 text-slate-900 font-semibold text-sm">
            <span className="flex items-center gap-2"><Layers className="w-4 h-4 text-indigo-500" /><span>טבלאות TOP 5</span></span>
            <ArrowLeft className="w-4 h-4 text-slate-400" />
          </Link>
          <Link href="/#deals" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-3 rounded-xl hover:bg-slate-50 text-slate-900 font-semibold text-sm">
            <span className="flex items-center gap-2"><Flame className="w-4 h-4 text-ali-500" /><span>דילים חמים</span></span>
            <ArrowLeft className="w-4 h-4 text-slate-400" />
          </Link>

          {top5Items.length > 0 && (
            <div className="pt-2 border-t border-slate-100 space-y-1">
              <div className="px-4 py-1 text-[11px] font-bold text-slate-400 uppercase tracking-wider">מדריכי השוואה פופולריים (TOP 5)</div>
              {top5Items.map((item) => (
                <Link key={item.id} href={item.href} onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-2.5 rounded-xl hover:bg-slate-50 text-slate-800 text-sm font-medium">
                  <span className="flex items-center gap-2.5"><span>{item.icon || "⭐"}</span><span>{item.title}</span></span>
                  <ArrowLeft className="w-3.5 h-3.5 text-slate-400" />
                </Link>
              ))}
            </div>
          ...
          {customHeaderLinks.length > 0 && (
            <div className="pt-2 border-t border-slate-100 space-y-1">
              <div className="px-4 py-1 text-[11px] font-bold text-slate-400 uppercase tracking-wider">ניווט נוסף</div>
              {customHeaderLinks.map((item) => (
                <div key={item.id} className="space-y-1">
                  <Link href={item.href} onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-2.5 rounded-xl hover:bg-slate-50 text-slate-800 text-sm font-medium">
                    <span className="flex items-center gap-2">{item.icon && <span>{item.icon}</span>}<span>{item.title}</span></span>
                    <ArrowLeft className="w-3.5 h-3.5 text-slate-400" />
                  </Link>
                  {item.isDropdown && item.children && item.children.length > 0 && (
                    <div className="mr-6 space-y-1 border-r border-slate-200 pr-3">
                      {item.children.map((child) => (
                        <Link key={child.id} href={child.href} onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between py-1.5 text-xs text-slate-600 hover:text-ali-600">
                          <span className="flex items-center gap-1.5">{child.icon && <span>{child.icon}</span>}<span>{child.title}</span></span>
                          <ArrowLeft className="w-3 h-3 text-slate-300" />
                        </Link>
                      ))}
                    </div>
          ...
          <Link href="/#customs-guide" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-3 rounded-xl bg-emerald-50/70 border border-emerald-200 text-emerald-900 font-semibold text-sm mt-2">
            <span className="flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-emerald-600" /><span>מדריך ומחשבון מכס ($75)</span></span>
            <ArrowLeft className="w-4 h-4 text-emerald-600" />
          </Link>
        </div>
      )}
